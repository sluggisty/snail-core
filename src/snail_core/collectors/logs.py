...
class LogsCollector(BaseCollector):
    ...
    def _get_boot_logs(self) -> list[dict[str, Any]]:
        ...
        stdout, _, rc = self.run_command([
            "journalctl", "-b", "-p", "warning", "-n", str(self.MAX_ENTRIES),
            "-o", "json", "--no-pager"
        ])
    ...
    def _get_auth_failures(self) -> dict[str, Any]:
        ...
        stdout, _, rc = self.run_command([
            ...
            "--since", since, "-g", "fail|invalid|error",
            "-o", "json", "--no-pager", "-n", "50"
        ])
    ...
    def _get_disk_errors(self) -> list[dict[str, Any]]:
        ...
        stdout, _, rc = self.run_command([
            "journalctl", "-k", "-n", "100",
            "-g", "error|fail|i/o|ata|scsi|nvme|sata",
            "-o", "json", "--no-pager"
        ])
                ...
                if line:
                    try:
                        ...
                        if any(kw in msg_lower for kw in [
                            "disk", "sda", "sdb", "nvme", "ata", "scsi",
                            "i/o error", "sector", "block", "drive"
                        ]):
                            entries.append({
                                "timestamp": self._format_journal_timestamp(
                                    entry.get("__REALTIME_TIMESTAMP")
