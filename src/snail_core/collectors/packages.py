...
class PackagesCollector(BaseCollector):
    ...
    def _get_dnf_upgradeable(self) -> dict[str, Any]:
        ...
        if stdout and rc in (0, 100):
            try:
                ...
                result["packages"] = [{"name": p.get("name", ""), "version": p.get("version", "")} 
    ...
    def _get_deb_kernel_packages(self) -> list[dict[str, str]]:
        ...
        kernels = []
        stdout, _, rc = self.run_command(["dpkg-query", "-W", "-f=${Package}|${Version}\n", "linux-image*"])
        if rc == 0 and stdout:
            for line in stdout.strip().split("\n"):
                if "|" in line:
    ...
    def _get_rpm_kernel_packages(self) -> list[dict[str, str]]:
        ...
        kernels = []
        stdout, _, rc = self.run_command(["rpm", "-qa", "kernel*", "--qf", "%{NAME}|%{VERSION}-%{RELEASE}\n"])
        if rc == 0 and stdout:
            for line in stdout.strip().split("\n"):
                if "|" in line:
                    name, version = line.split("|", 1)
